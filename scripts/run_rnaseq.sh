...
#SBATCH -o rnaseq_%A_%a.o.txt    # %A is job ID, %a is array task ID
#SBATCH -e rnaseq_%A_%a.ERROR.txt
...
export NXF_WORK="/bigdata/cosmelab/lcosme/docker/work"
export NXF_TEMP="/bigdata/cosmelab/lcosme/docker/temp"
export NXF_OPTS="-Xms512m -Xmx4g"
export NXF_SINGULARITY_CACHEDIR="/bigdata/cosmelab/lcosme/docker/singularity"
...
FASTA="/bigdata/cosmelab/lcosme/docker/data/references/albo.fasta.gz"
GTF="/bigdata/cosmelab/lcosme/docker/data/references/genes_fixed.rsem.fixed.gtf"
...
nextflow run nf-core/rnaseq \
    ...
    --outdir results/${PROJECT}/${SAMPLE_ID} \
    --fasta "$FASTA" \
    --gtf "$GTF" \
...
if [ $? -eq 0 ] && [ -d "results/${PROJECT}/${SAMPLE_ID}" ]; then
